...
namespace {
    ...
    public:
        constexpr explicit ComparableRegularTest(T test) noexcept: value_(std::move(test)) {}

        // NOLINTNEXTLINE(hicpp-use-nullptr, modernize-use-nullptr)
        constexpr auto operator<=>(const ComparableRegularTest&) const noexcept = default;

        [[nodiscard("You call the function to get the value.")]] constexpr T value() const noexcept { return value_; }
...
DOCTEST_TEST_SUITE("Test ComparableRegular concept") {
    DOCTEST_TEST_CASE_TEMPLATE("Test on an integer",
                               T,
        ...
        DOCTEST_SUBCASE("Equality") {
            ...
            DOCTEST_REQUIRE((::ComparableRegularTest(integer) != ::ComparableRegularTest(static_cast<T>(integer + 1))));
        }

        DOCTEST_SUBCASE("Simple operator") {
            DOCTEST_REQUIRE((::ComparableRegularTest(static_cast<T>(integer + 1)) > ::ComparableRegularTest(integer)));
            DOCTEST_REQUIRE((::ComparableRegularTest(integer) < ::ComparableRegularTest(static_cast<T>(integer + 1))));
        }
    ...
    DOCTEST_TEST_CASE_TEMPLATE("Test on an integer value",
                               T,
                               COMMON_CONCEPTS_TEST_SIGNED_INTEGERS,
                               COMMON_CONCEPTS_TEST_UNSIGNED_INTEGERS) {
        constexpr T integer = 15;
        ...
        DOCTEST_SUBCASE("Equality") {
            ...
            DOCTEST_REQUIRE((::ComparableRegularTest(integer).value()
                             != ::ComparableRegularTest(static_cast<T>(integer + 1)).value()));
        }

        DOCTEST_SUBCASE("Simple operator") {
            DOCTEST_REQUIRE((::ComparableRegularTest(static_cast<T>(integer + 1)).value()
